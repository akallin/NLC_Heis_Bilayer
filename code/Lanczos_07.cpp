...
// Reduces the Hamiltonian Matrix to a tri-diagonal form 
{
    ...
    //Matrices
    vector<l_double> alpha(LIT, 0.);
    vector<l_double> beta(LIT, 0.);
...
//        if( V0.size() == 4) V0[1] = 1.0;
        for (unsigned long int vi=0; vi < V0.size(); vi++) 
        {
            V0[vi] = 0;
            if(vi%5==0){V0[vi]=1;}
            else if(vi%6==0){V0[vi]=2;}
            else if(vi%7==0){V0[vi]=-3;}
        }
        V0[0] = 2.0;
        V0[V0.size()-1] = -2.0;
        V0[V0.size()/2] = 1.0;
...
int LANCZOS::tqli2(vector<l_double >& d, vector<l_double>& e, int n, vector< vector< l_double > >& z, const int Evects)
/***
